...
from models import Role, Workflow, State, Transition, EventType, Event, WorkflowActivity, WorkflowHistory
...
class WorkflowActivityAdmin(admin.ModelAdmin):
    list_display = ['id','workflow', 'created_on', 'created_by', 'current_state']

class WorkflowHistoryAdmin(admin.ModelAdmin):
    list_display = ['workflowactivity','log_type', 'state', 'transition', 'event', 'participant', 'note']
...
admin.site.register(WorkflowActivity, WorkflowActivityAdmin)
